...
export const GlobalProvider = ({ children }) => {
    ...
    const [loadingProgress, setLoadingProgress] = useState(0);//: the current value of the state(loadingProgress), and a function to update the state(setLoadingProgress).
    const loadingSpeedController = () => {   //setCount is defined by the useState hook in React. The useState hook returns an array of two values: the current state value and a function to update the state value. The second value in the array is the function that you can use to update the state.

        setTimeout(() => {
            setLoadingProgress();//30 % done after 200ms
        }, );
        ...
        setTimeout(() => {
            LoadingBar.finish();//finished after 1700ms
        }, );
